 ...
 *
 * Four directories in the open project's folder — `briefs/`, `status/`,
 * `artefacts/`, `decisions/` — with one button to lay down whatever is missing.
 * That is the whole surface. The files themselves are read and written by the
 * conversation, through `fs_write` and the approval dialog, which is why there
 * is no editor here: a panel that could rewrite `DECISIONS.md` without passing
 * the gate would be a second write path around it.
 *
    ...
    <section className="shared" aria-label="Shared workspace files">
      ...
      {layout.complete ? (
        <p className="shared__note">
          Briefs, status, artefacts and decisions are in this folder. Ask for a
          decision to be recorded and it goes in <code>DECISIONS.md</code>.
        </p>
      ) : (
        <>
          ...
          <button
            ...
            onClick={() => void scaffold(project.id)}
            disabled={busy}
